...
- **二维码标签**：每个收纳箱可生成 `findit://box/{slug}` 二维码，可打印张贴；App 内扫码直达箱内清单，外部相机扫码（Android/iOS 已注册 `findit://` 深链）也可直接唤起 App 并跳转到对应收纳箱
...
- **备份/恢复**：数据库 + 照片一键打包为 zip 导出；恢复采用原子替换，失败自动回滚，
  旧数据保留为副本（仅保留最近一份）
...
- **导出**：设置页一键将数据库与照片打包为 zip，经系统分享面板保存到目标位置
  （文件管理器 / 网盘等）；若取消分享，zip 会保留在应用备份目录并在提示中
  显示完整路径，可稍后重新分享，不会因取消而丢失。AI API Key 不会写入备份文件
- **恢复**：选择备份 zip 恢复；校验通过后原子替换数据目录，旧数据保留为副本
  （仅保留最近一份），任一环节失败自动回滚，不损坏现有数据
